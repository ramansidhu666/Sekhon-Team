...
namespace Property
{
    ...
    public class Newsletterfile : System.Web.Services.WebService
    {
        ...
        [System.Web.Services.WebMethod]
        [ScriptMethod(UseHttpGet = false)]
        public string PrintLetterHead(string LetterHeadId)
        {
            SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ConStr"].ToString());
            string html = "";
            ...
            try
            {
                ...
                DataTable dt = new DataTable();
                string str = "select * from LetterHead where LetterHeadId=" + LetterHeadId;
                SqlDataAdapter adp = new SqlDataAdapter(str, conn);
                adp.Fill(dt);
                if (dt.Rows.Count > 0)
                {
        ...
        [System.Web.Services.WebMethod]
        [ScriptMethod(UseHttpGet = false)]
        public string PrintEnvelop(string EnvelopId)
        {
            SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ConStr"].ToString());
            string html = "";
            ...
            try
            {
                ...
                DataTable dt = new DataTable();
                string str = "select * from Envelop where EnvelopId=" + EnvelopId;
                SqlDataAdapter adp = new SqlDataAdapter(str, conn);
                adp.Fill(dt);
                if (dt.Rows.Count > 0)
                {
